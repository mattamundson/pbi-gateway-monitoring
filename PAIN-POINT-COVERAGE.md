...
> and `starter/tests/`. Nothing here has run in a real Fabric tenant yet — that is the
> community validation step (see `QUICKSTART.md` / `CONTRIBUTING.md`).
...
|---|---|---|---|---|
| 4 | **PBIT breaks on gateway upgrade** | Schema-adaptive parser `gateway_bronze_lib.read_gateway_csv` (name-based, PERMISSIVE, mergeSchema) | 🟢 **PROVEN** | Both test tiers pass on real Spark; survives mid-schema column injection where MS PBIT throws `DataFormat.Error`. |
...
| 3 | **No query→identity attribution (flagship)** | `01_identity_join.kql` (RequestId↔XmlaRequestId) + `add_artifact_identity` | 🟠 **BUILT / UNPROVEN** | Extraction tested on synthetic data; the tenant join to Workspace Monitoring is THE unproven claim — validate first. |
...
|---|---|---|
| 🟢 Proven (incl. local) | **3** | #4, #5, #6 |
| 🟡 Built + local-tested | **1** | #9 |
| 🟠 Built, needs tenant | **6** | #1, #2, #3, #7, #8, #10 |
| 🔴 Not addressed | **0** | — |

**Bottom line:** all 10 are *addressed by design*; **4 are proven on real Spark / synthetic data**; **6 have real code that only a live-tenant pilot can validate.** The single gate that flips most of the 🟠 items to proven is the identity-join pilot (#3) plus a real gateway feeding the collectors.
...
visibility (#5) outright; the rest are implemented and pending live-tenant validation."*
