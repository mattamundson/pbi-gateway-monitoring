...
| 3 | **No query→identity attribution (flagship)** | `01_identity_join.kql` (RequestId↔XmlaRequestId) + `add_artifact_identity`; **`04_identity_match_rate.kql`** (throttle-safe match-rate measurement, Blocks A/B/C) | Join **key**: `[Desk-Verified]` (confirmed in MS docs). Live **proof**: 🟠 **BUILT / UNPROVEN — blocker confirmed** | **Live-tenant confirmed blocker:** Workspace Monitoring Eventhouse fails to provision on trial capacity; needs paid **F2+**. **Session 4:** narrow-window workaround + a dedicated match-rate query (`04_identity_match_rate.kql`) now make the first green run yield the U11 % directly, not just "rows returned." See `LIVE-TENANT-FINDINGS.md` → 2026-07-01 + 2026-07-02 Sessions 3–4. |
