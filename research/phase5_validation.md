...
> **Provenance of U14–U17:** surfaced by 2026-07-01 static code review + end-to-end contract tracing, not yet reproduced on a live host. They are code-quality/correctness items (not new unknowns about the platform) — fold their fixes in during the pilot, and add a regression case to `starter/tests/` where the logic is Spark-free.
>
...
> - **U17 — FIXED (compile-checked; behavior is documented Spark semantics).** Found by tracing the collector→ingest contract end to end: all six JSON bronze ingests called `spark.read.json` with the default `multiLine=false`, but six of the seven collectors emit pretty-printed multi-line `ConvertTo-Json` output. Spark's default JSONL mode would parse each physical line separately → `_corrupt_record` → **every one of those six ingests would fail on real collector data.** The tell: `Collect-MashupProcesses` is the only collector emitting JSONL (`ConvertTo-Json -Compress` per line), which is exactly why the mashup path is the only JSON ingest the local smoke test exercises successfully. Fixed by adding `.option("multiLine", True)` to all six reads. This is documented Spark behavior (not speculative), but the reads run only in a Spark session, so it is compile-checked here and marked for pilot confirmation. **New local coverage:** added a `refresh_history_synthetic.json` fixture (generator: `gen_refresh_history()`, produces the collector's pretty wrapper shape — validated: 18 records, all triage-join columns present, 16 RequestIds overlap the QE space) and a `run_local_smoke.py` **STEP 7** that reads it via the same `multiLine=True` + `explode(RefreshRecords)` path and runs the Level-1 exact-`RequestId` triage join — so the pain #2 refresh leg, previously in the smoke test's *NOT exercised* list, now gets exercised when a tester runs the smoke test with a JDK present.
