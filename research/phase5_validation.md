...
| U19 | **Exact Workspace-Monitoring identity join** (`RequestId == OperationId`, `01_identity_join.kql` core `kind=inner` join) | `01_identity_join.kql` core join (~L66-95); test harness `starter/kql/PILOT-identity-join-test.kql` | Run `PILOT-identity-join-test.kql` blocks 1–2 (and the narrowed block 4 scan) against a live Workspace Monitoring Eventhouse with gateway `QueryStart`/`QueryExecution` logs landed; confirm `OperationId == RequestId` returns populated `ExecutingUser`/`ItemName` for real gateway RequestIds | If the join returns empty despite prerequisites being met: re-check whether `OperationId` is truly identical to the gateway `RequestId` in this tenant (see `01_identity_join.kql` header sourcing); if not, fall back to the time-window fuzzy correlation already used for U11 |
...
- [ ] Every U-item (U1–U19) is confirmed or has a documented fix applied.
